...
public class TreatmentPediatricBrainTumor {
	private String a;
	private String b;    
		
	public String Symptoms1() throws IOException {

	    Document document = Jsoup.connect("https://www.mayoclinic.org/diseases-conditions/pediatric-brain-tumor/diagnosis-treatment/drc-20361706").timeout(6000).get();
	...
	public String Symptoms2() throws IOException {
	    ...
	    Document document = Jsoup.connect("https://www.mayoclinic.org/diseases-conditions/pediatric-brain-tumor/diagnosis-treatment/drc-20361706").timeout(6000).get();
	    Elements symppara1 = document.select("div.main article[id*=main-content] div.row div.content div h3");
	    for (int i = 1; i < 2; i++){
			 String c = symppara1.get(i).text();
			 return c;
		}
